...
------------------------------------------------------------------------------------------------ */
// try using match instead of pushing to new array ****
...
const citiesAtoJ = (arr) => {
  ...
  let regex = /^[A-J]/g;
  arr.forEach(city => {
    if(regex.test(city)) { // not sure why this isn't adding 'Boston'
      rtn.push(city);
    }
  })
  return rtn;
...
const matchMonth = (input) => {
  let regex = /^(October)$|^(Oct)$|^(october)$|^(oct)$/g; // need to delete bar somehow?
  return regex.test(input);
};
