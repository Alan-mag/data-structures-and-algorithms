...
const countNumberOfElements = (arr) => {
 let count = arr.reduce((accumulator, ele, idx) => {
...
const returnNames = (arr) => {
  let names = arr.reduce((acc, ele, idx) => {
    acc.push(ele.name);
    return acc;
  }, [])
  return names;
...
const reversedString = (arr) => {
  let rev = arr.split('').reduce((acc, ele, idx) => {
    return ele + acc;
  }, '');
  return rev;
};
...
const countNumberOfChildren = (arr) => {
  let numChildren = arr.reduce((acc, fam, idx) => {
    if (fam.children) {
      acc = acc+=fam.children.length;
    }
    return acc;
  }, 0);
  return numChildren;
...
const calculateAverage = (arr) => {
  let data = arr.reduce((acc, num, idx) => {
    acc.count++;
    acc.sum += num;
    return acc;
  }, { count: 0, sum: 0 });
...
const countPrimeNumbers = (arr) => {
  let primes = arr.reduce((acc, ele, idx) => {
    acc = isPrime(ele) ? acc+=1 : acc; // why doesn't acc++ work?
    return acc;
  }, 0);
  return primes;
}
...
const extractStat = (statName, arr) => {
  let matched = arr.reduce((acc, ele, idx) => {
    acc = ele.stat.name === statName ? acc = ele : acc;
    return acc;
  }, {});
  return matched;
};
...
const extractChildren = (arr) => {
  let matchedLetter = arr.filter((ele, i) => {
    if(ele.name.includes('a')) {return true;}
  })
  let allChildren = matchedLetter.reduce((acc, ele, idx) => {
    acc = ele.children ? acc.concat(ele.children) : acc;
    return acc;
  }, []);
