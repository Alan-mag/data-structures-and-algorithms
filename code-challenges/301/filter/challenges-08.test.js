...
const filterStringsWithVowels = (arr) => {
  let vowelWords = arr.filter((word, i) => {
...
const notInFirstArray = (forbiddenValues, arr) => {
  let rtnArr = arr.filter((ele, i) => {
    return (forbiddenValues.indexOf(ele) >= 0) ? null : ele;
  })
  return rtnArr;
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let rtn = [];
  let arr2 = arr.filter((obj, i) => {
    return (obj.baseStat > minBaseStat) ? obj.stat.name : null;
  })
  arr2.forEach((ele) => {
    rtn.push(ele.stat.name)
  })
  return rtn;
};
